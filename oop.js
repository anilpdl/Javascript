...
class Game {
	...
	_assignedCardsStrength(playerHandCardsValue,playerHandCardsSuit){
		...
		console.log(playerHandCardsValue[0] ,playerHandCardsValue[1], playerHandCardsValue[2]);
	...
	checkWinner(results){
		...
		var winnerArray =[];
		var indexOfMaxValue = max(results);
		if(indexOfMaxValue.length==1){
			return indexOfMaxValue;
		}

		for(var i=1;i<indexOfMaxValue.length;i++){
			var compareResult = this.equalResultsCardcompare(i,i-1);
			if(compareResult==1){
				var temp = indexOfMaxValue[i];
				indexOfMaxValue[i] = indexOfMaxValue[i-1];
				indexOfMaxValue[i-1] = temp;
				var equal = 0;
			}
			else if(compareResult==0){
				var equal = 1;
				winnerArray.push(indexOfMaxValue[i-1],indexOfMaxValue[i]);
			}
		}
		if(equal==1){
			return winnerArray;
			...
			return winnerArray.push(indexOfMaxValue[0]);
		}
	}
